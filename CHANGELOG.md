...
- **Security/Gateway**: 阻止向非环回地址发起明文 WebSocket (`ws://`) 连接；非环回绑定（tailnet/lan）需要 TLS 或 `wss://`（CWE-319）
- **Security/iMessage**: 通过要求严格主机密钥验证、将 `channels.imessage.remoteHost` 验证为 `host`/`user@host`，以及拒绝配置或自动检测中的不安全主机令牌来加固远程附件 SSH/SCP 处理
- **Security/Skills**: 在技能打包期间拒绝符号链接，防止在分发的 `.skill` 档案中包含外部文件
- **Security/Feishu+LINE**: 通过共享 `buildRandomTempFilePath` 辅助函数集中强化的临时文件路径生成，降低漂移风险
- **Security/YAML**: 在 frontmatter 解析中使用 YAML core schema 以防止类型强制转换
