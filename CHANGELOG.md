...
> 📡 **上游同步**：同步上游 v2026.2.17更新，移植 security-part7 安全加固

### 🔒 安全加固（上游 security-part7 移植）

- **Security/Elevated**：`tools.elevated.allowFrom` 现在只匹配发送方身份字段（`SenderId`、`SenderE164`、`From`），不再匹配收件人 `ctx.To`，关闭了通过收件人 token 绕过 `/elevated` 授权的漏洞。可变发送方字段（`SenderName`、`SenderUsername`、`SenderTag`）需要使用显式前缀（`name:`、`username:`、`tag:`）才能匹配。（同步上游 commit 6817c0ec7b4f、02772b029d6d）
- **Security/Group policy**：加固 `channels.*.groups.*.toolsBySender` 匹配，要求使用显式发送方键类型（`id:`、`e164:`、`username:`、`name:`），防止跨可变字段碰撞攻击，同时保留旧版无前缀键的废弃兼容路径（仅匹配 ID）。（同步上游 commit 5547a2275cb6）
