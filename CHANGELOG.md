...
- **提示路径注入加固**：清理工作区路径中的 Unicode 控制/格式字符后再嵌入 LLM 提示，防止恶意目录名触发指令注入（upstream `93b9f1ec5`）感谢 @aether-ai-agent
- **会话溯源安全**：为路由提示保留跨会话输入溯源，防止委派/内部会话被误判为直接外部用户指令（upstream `e3445f59c`）感谢 @anbecker
