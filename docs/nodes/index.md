...
- `--display-name` on `clawdbot node run` / `clawdbot node install` (persists in `~/.openclaw/node.json` on the node).
...
- macOS nodes drop `PATH` overrides and strip dangerous startup/shell keys (`DYLD_*`, `LD_*`, `NODE_OPTIONS`, `PYTHON*`, `PERL*`, `RUBYOPT`, `SHELLOPTS`, `PS4`); headless node hosts only accept `PATH` when it prepends the node host PATH.
