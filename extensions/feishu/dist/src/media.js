...
export async function downloadMessageResourceFeishu(params) {
    ...
    const responseAny = response;
    if (responseAny.code !== undefined && responseAny.code !== 0) {
        throw new Error(`Feishu message resource download failed: ${responseAny.msg || `code ${responseAny.code}`}`);
    }
    // Handle various response formats from Feishu SDK
    let buffer;
    if (Buffer.isBuffer(response)) {
        ...
        const keys = Object.keys(responseAny);
        const types = keys.map((k) => `${k}: ${typeof responseAny[k]}`).join(", ");
        throw new Error(`Feishu message resource download failed: unexpected response format. Keys: [${types}]`);
    }
    return { buffer };
}
/**
