...
export function createHooksRequestHandler(
  ...
  const hookRateLimiter = createAuthRateLimiter({
    maxAttempts: 20,
    windowMs: 60_000,
  });
  ...
  return async (req, res) => {
    ...
    if (!safeEqualSecret(token, hooksConfig.token)) {
      const throttle = hookRateLimiter.record(clientKey);
      if (throttle.throttled) {
        ...
        logHooks.warn(
          `hook auth throttled for ${clientKey} (${AUTH_RATE_LIMIT_SCOPE_HOOK_AUTH}); retry-after=${retryAfter}s`,
        );
        return true;
      }
      res.statusCode = 401;
    ...
    hookRateLimiter.clear(clientKey);
