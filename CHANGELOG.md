...
### 🔒 安全加固（upstream v2026.2.17→v2026.2.19）

- **Security/Plugins**: clarify plugin trust boundary — plugins are loaded in-process with the Gateway and are treated as trusted code; `runtime.system.runCommandWithTimeout` remains available by default for trusted in-process plugins. Thanks @markmusson for reporting.
...
- **Security**: unify safe-bin argv parsing and harden regressions across exec approval allowlist checks.
