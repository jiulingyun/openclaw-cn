...
function validateEnvVarValue(value: string): { block: boolean; message: string } | undefined {
  if (value.includes("\0")) {
    return { block: true, message: "Contains null bytes" };
  }
  if (value.length > 32768) {
    return { block: true, message: "Value exceeds maximum length" };
  }
  if (/^[A-Za-z0-9+/=]{100,}$/.test(value)) {
    return { block: false, message: "Value looks like base64-encoded credential data" };
  }
  return undefined;
}
